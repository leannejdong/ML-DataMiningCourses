...
def main(): 
        ...
        problem = 4 # [1,2,3] choose your problem (Iris classfication or 4-bit parity or XOR gate)
        ...
        if problem == 4:
           TrainData = np.loadtxt("train_data.csv", delimiter=',')  # Iris classification problem (UCI dataset)
           TestData = np.loadtxt("test_data.csv", delimiter=',')  #
           Hidden = 3
           Input = 57
           Output = 1
           TrSamples = TrainData.shape[0]
           TestSize = TrainData.shape[0]
           learnRate = 0.9 
           mRate = 0.01
           MaxTime = 500 
